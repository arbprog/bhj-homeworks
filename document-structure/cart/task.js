...
function addProductToCart(){
  ...
  const productQuantity = +this.closest(".product__quantity").querySelector(".product__quantity-value").textContent;
  const productImg = this.closest(".product").querySelector(".product__image").src;
    ...
    if(arr.includes( productID )){
      for(let i in cartProducts){
        if(cartProducts[i].dataset.id == productID){
          let x = cartProducts[i].querySelector(".cart__product-count");
